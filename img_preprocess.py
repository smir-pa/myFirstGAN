...
def crop_img(coords, in_dir, out_dir):
    ...
    for index, row in data.iterrows():
        ...
        cropped = img.crop((row['x'], row['y'], row['x'] + row['size'], row['y'] + row['size']))
        # cropped = cropped.resize((image_size, image_size))
        if not os.path.exists(out_dir + '/' + row['name']):
            cropped.save(out_dir + '/' + row['name'])
        else:
            cropped.save(out_dir + '/' + row['name'][0:len(row['name'])-4] + '_1.jpg')
...
if __name__ == '__main__':
    ...
    # crop_img('res/training_imgs/nodules_coords.csv', 'res/training_imgs/nodules_jpg', 'res/training_imgs/nodules_cropped/1')
